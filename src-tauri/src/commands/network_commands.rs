...
pub async fn get_networks() -> Result<Vec<DockerNetwork>> {
    ...
    let networks = client.list_networks(options).await
        .map_err(|e| DockerError::OperationFailed { message: format!("Failed to list networks: {}", e) })?;
    ...
    for network in networks {
        let ipam = network.ipam.map(|ipam| NetworkIPAM {
            ...
            config: ipam.config.map(|configs| {
                configs.into_iter().map(|config| NetworkIPAMConfig {
...
pub async fn get_network_details(id: String) -> Result<DockerNetwork> {
    ...
    let network = client.inspect_network(&id, None::<bollard::network::InspectNetworkOptions<String>>).await
        .map_err(|e| DockerError::OperationFailed { message: format!("Failed to inspect network: {}", e) })?;
...
pub async fn remove_network_cmd(id: String) -> Result<()> {
    ...
    client.remove_network(&id).await
        .map_err(|e| DockerError::OperationFailed { message: format!("Failed to remove network: {}", e) })?;
...
pub async fn prune_networks_cmd() -> Result<()> {
    ...
    client.prune_networks(None::<bollard::network::PruneNetworksOptions<String>>).await
        .map_err(|e| DockerError::OperationFailed { message: format!("Failed to prune networks: {}", e) })?;
