...
<script lang="ts">
  import { connectionStatus, isConnecting, dockerError } from '$lib/stores/docker.svelte';
  import { containers, isLoadingContainers } from '$lib/stores/containers.svelte';
  ...
  let currentResolvedTheme = $derived(resolvedTheme());
  ...
  <CardContent>
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-4 text-xs">
      ...
      <div class="space-y-2">
        ...
        <div class="space-y-1 text-muted-foreground">
          <div>Updated: {new Date(debugInfo.app.timestamp).toLocaleTimeString()}</div>
