...
import type { Network, NetworkInspect, CreateNetworkOptions, ConnectNetworkOptions, DisconnectNetworkOptions } from '../types/network';
...
async function connectNetwork(networkId: string, options: ConnectNetworkOptions): Promise<void> {
  return await invoke('connect_network_cmd', { networkId, options });
}

async function disconnectNetwork(networkId: string, options: DisconnectNetworkOptions): Promise<void> {
...
class NetworksStore {
  ...
  get sortedNetworks() {
    ...
    sorted.sort((a, b) => {
      switch (this._networkSortBy) {
        ...
        case 'containers':
          return this.getNetworkContainerCount(b) - this.getNetworkContainerCount(a); // Most used first
        case 'scope':
      ...
      switch (action) {
        ...
        case 'connect':
          if (typeof networkIdOrOptions !== 'string' || !additionalOptions) {
            throw new Error('Connect action requires network ID and ConnectNetworkOptions');
          }
          await connectNetwork(networkIdOrOptions, additionalOptions as ConnectNetworkOptions);
          break;
        case 'disconnect':
          if (typeof networkIdOrOptions !== 'string' || !additionalOptions) {
            throw new Error('Disconnect action requires network ID and DisconnectNetworkOptions');
          }
          await disconnectNetwork(networkIdOrOptions, additionalOptions as DisconnectNetworkOptions);
          break;
        case 'prune':
          await pruneNetworks();
          break;
  ...
  async disconnectNetwork(networkId: string, options: DisconnectNetworkOptions) {
